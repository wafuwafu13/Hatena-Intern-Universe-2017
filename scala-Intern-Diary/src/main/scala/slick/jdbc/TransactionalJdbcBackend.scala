...
trait TransactionalJdbcBackend extends JdbcBackend {
  class TransactionalDatabaseDef(source: JdbcDataSource, executor: AsyncExecutor = AsyncExecutor.default())
      extends super.DatabaseDef(source, executor) {
    override def createSession(): Session = new TransactionalSession(this)

    class TransactionalSession(database: Database) extends BaseSession(database) {
