...
import internbookmark.service.{BookmarkApp, BookmarkAppImpl, Context}
...
object BookmarkCli {
  ...
  def createApp(userName: String): BookmarkApp = new BookmarkAppImpl(userName)
  ...
  def init_db(app: BookmarkApp)(implicit ctx: Context): Int = {
    app.createTables()
    0
  }
  ...
  private[this] def prettifyBookmark(bookmark: Bookmark): String =
    bookmark.entry.title + " (" + bookmark.entry.url + ") " + (if (bookmark.comment == "")
                                                                 ""
                                                               else
                                                                 " " + bookmark.comment)
}
