...
trait BookmarkAPIWeb extends JacksonJsonSupport {
  self: BookmarkWeb with AppContextSupport =>
  ...
  post("/api/bookmark") {
    ...
    req match {
      ...
      case Left(errorResult) =>
        errorResult.copy(body = JObject("error" -> JString(errorResult.status.message)))
    }
  }
}
