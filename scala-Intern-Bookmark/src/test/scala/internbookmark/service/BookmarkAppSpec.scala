...
class BookmarkAppSpec extends UnitSpec with SetupDB {
  private def createApp(): BookmarkApp =
    new BookmarkApp {
      override val currentUserName: String = Random.nextInt().toString
      override def titleExtractor: TitleExtractor =
        new TitleExtractor {
          def fetch(url: String): Option[String] =
            Some("<title>test-title</title>")
        }
    }
  ...
  describe("BookmarkApp") {
    ...
    it("should save data when new bookmark with a comment is added") {
      val app = createApp()
      app
        .add("http://www.example.com", "very cool website!!")
        .fold({ _ =>
          fail()
        }, { bookmark =>
