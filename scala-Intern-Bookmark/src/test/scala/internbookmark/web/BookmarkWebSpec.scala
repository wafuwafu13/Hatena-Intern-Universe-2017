...
import internbookmark.service.{BookmarkApp, TitleExtractor}
import internbookmark.repository
import internbookmark.model._
import internbookmark.helper.Factory._
import internbookmark.helper.{SetupDB, WebUnitSpec}

import scala.util.Random

class BookmarkWebForTest extends BookmarkWeb {
  override def createApp()(implicit request: HttpServletRequest): BookmarkApp =
    new BookmarkApp {
      override def titleExtractor = new TitleExtractor {
        override protected def fetch(url: String) = Some("<title>stub page</title>")
      }

      override val currentUserName: String = BookmarkWebForTest.this.currentUserName()
    }
...
class BookmarkWebSpec extends WebUnitSpec with SetupDB {
  ...
  describe("BookmarkWeb") {
    ...
    it("should redirect to the list page when the top page is accessed") {
      get("/", headers = withUserSessionHeader()) {
        status shouldBe 302
    ...
    it("should show list of bookmarks") {
      get("/bookmarks", headers = withUserSessionHeader()) {
        status shouldBe 200
      }
    }

    it("should show a page to add a bookmark") {
      get("/bookmarks/add", headers = withUserSessionHeader()) {
        status shouldBe 200
      }
    }
    ...
    it("should make new bookmark when the bookmark with an empty comment is posted") {
      ...
      post("/bookmarks/add", params = List("url" -> url, "comment" -> ""), headers = withUserSessionHeader()) {
        status shouldBe 302
        header.get("Location") should contain("/bookmarks")
        ...
        val maybeBookmark =
          repository.Bookmarks.findByEntry(testUser(), maybeEntry.get)
    ...
    it("should make new bookmark when the bookmark with an non-empty comment is posted") {
      ...
      post("/bookmarks/add", params = List("url" -> url, "comment" -> "hello~"), headers = withUserSessionHeader()) {
        status shouldBe 302
        header.get("Location") should contain("/bookmarks")
    ...
    it("should show a page to edit a bookmark") {
      ...
      val bookmark =
        createBookmark(Some(testUser()), Some(entry), comment = "not changed")

      get(s"/bookmarks/${bookmark.id}/edit", headers = withUserSessionHeader()) {
        status shouldBe 200
      }
    }
    ...
    it("should show a page to delete a bookmark") {
      ...
      get(s"/bookmarks/${bookmark.id}/delete", headers = withUserSessionHeader()) {
        status shouldBe 200
      }
    }
    ...
    it("should delete a bookmark specified by the url") {
      ...
      post(s"/bookmarks/${bookmark.id}/delete", params = List(), headers = withUserSessionHeader()) {
        status shouldBe 302
    ...
    it("should return a json representation of list of bookmarks") {
      ...
      get("/api/bookmarks", params = List(), headers = withUserSessionHeader()) {
        status shouldBe 200
    ...
    it("should add a new bookmark and return its json representaion") {
      ...
      post(
        "/api/bookmark",
        body = JsonMethods.compact(JsonMethods.render(JObject("url" -> JString(url), "comment" -> JString("hello!")))),
        headers = withUserSessionHeader()
      ) {
        status shouldBe 200
        inside(header.get("Content-Type")) {
    ...
    it("should respond 400 with invalid id") {
      val reqs = Seq[(=> Unit) => Unit](
        get[Unit]("/bookmarks/hoge/edit", headers = withUserSessionHeader()),
        get[Unit]("/bookmarks/hoge/delete", headers = withUserSessionHeader()),
        post[Unit]("/bookmarks/hoge/edit", params = List(), headers = withUserSessionHeader()),
        post[Unit]("/bookmarks/hoge/delete", params = List(), headers = withUserSessionHeader())
      )
      reqs.foreach { req =>
        req {
          status shouldBe 400
    ...
    it("should respond 404 with non-existing id") {
      ...
      val reqs = Seq[(=> Unit) => Unit](
        get(s"/bookmarks/$bookmarkId/edit", headers = withUserSessionHeader()),
        get(s"/bookmarks/$bookmarkId/delete", headers = withUserSessionHeader()),
        post(s"/bookmarks/$bookmarkId/edit", params = List(), headers = withUserSessionHeader()),
        post(s"/bookmarks/$bookmarkId/delete", params = List(), headers = withUserSessionHeader())
      )
      reqs.foreach { req =>
        req {
          status shouldBe 404
